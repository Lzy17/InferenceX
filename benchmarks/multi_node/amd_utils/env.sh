...
# MoRI-specific env vars (only needed when using mori transfer backend)
if [[ "${KVTRANSFER_BACKEND:-mori}" == "mori" ]]; then
    ...
    echo "[INFO] KVTRANSFER_BACKEND=${KVTRANSFER_BACKEND:-mooncake} — skipping MoRI env vars"
fi
...
# QoS/DSCP configuration (MoRI-specific)
if [[ "${KVTRANSFER_BACKEND:-mori}" == "mori" ]]; then
