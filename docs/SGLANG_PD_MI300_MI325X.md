...
|--------|----------------|
| `IBDEVICES` empty / detect script fails | Install RDMA userspace tools on the launcher; or set `IBDEVICES` manually to match `ibv_devinfo`. |
| libbnxt / kernel ABI warnings in container | Tarball version must match host `bnxt_re` module on **that** node (MI300X vs MI325X often differ); try the alternate `libbnxt_re-*.tar.gz` per `rebuild_bnxt.sh` comments. |
| MoRI clone fails in container | GitHub access from nodes; or `INSTALL_MORI_MODE=path` and mount ROCm/mori at `MORI_SOURCE_PATH`. |
| Connection timeouts between nodes | Firewall, wrong data-plane IP, or routing; verify `IPADDRS` in logs matches the intended NICs. |
| Permission denied Docker | `USE_SUDO_FOR_DOCKER` vs docker group. |
| Wrong GPU / poor performance | Confirm `IMAGE` and `GPUS_PER_NODE` / TP settings match MI300X (8×GPU node) vs MI325X expectations and SGLang docs for that stack. |
